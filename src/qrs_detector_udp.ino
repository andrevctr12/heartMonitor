...
// WIFI
const char *ssid = "PTI-WIFI"; //  your network SSID (name)
...
//TODO mudar de lugar, criar libs
void initWiFi(void);
void reconectWiFi(void);
void VerifyWiFi(void);
...
void reconectWiFi()
{
    ...
    // Configura a conexão wifi (AP)
    WiFi.begin(ssid, pass);

    // Espera para conectar no AP
    ...
    Serial.print("UDP server na porta: ");
...
void UDP_send(float bpm)
{
    ...
    Serial.println("Batimentos enviados para o servidor");
    Serial.println();
}
...
void UDP_sendRaw(int val)
{
    ...
    Serial.println("Leitura completa enviada para o servidor");
    Serial.println();
}
void MQTT_send(float bpm) {
    if ((millis() - lastMQTTSendTime) > INTERVALO_ENVIO_MQTT)
    {
        MsqMQTT[]
        sprintf(MsgMQTT,"");
        MQTT.publish(TOPICO_PUBLISH, MsgMQTT);
        lastMQTTSendTime = millis();
    }
    Serial.println("Batimentos enviados para o servidor MQTT");
    Serial.println();
}

void MQTT_sendRaw(int val) {

    Serial.println("Leitura completa enviada para o servidor MQTT");
    Serial.println();
}
